...
**Prerequisites:** Module 00 complete, Ollama running, `gemma3:12b` pulled.
...
```bash
ollama run gemma3:12b "Write a one-sentence job task description for a machine operator who inspects weld joints."
```
...
```bash
ollama run gemma3:12b "Complete this sentence with exactly 5 words: The operator must inspect the"
```
...
```bash
ollama run gemma3:12b "Complete this sentence with exactly 5 words: The welder must verify the"
```
...
```bash
ollama run gemma3:12b "Break the word 'manufacturing' into syllables."
```
...
```bash
ollama run gemma3:12b "Write one sentence describing a weld inspection task."
```
...
For `gemma3:12b`, the context window is 128K tokens by default, but in practice, Ollama often defaults to a smaller window for speed. Let's see what this means concretely.

Run this:

```bash
ollama run gemma3:12b "I'm going to tell you a secret code: BLUE-FALCON-42. Remember it. What's 2+2?"
```

It answers the math question. Now ask:

```bash
ollama run gemma3:12b "What was the secret code I told you?"
```
...
```bash
ollama run gemma3:12b "What is specification WPS-9999-XYZ at Acme Manufacturing?"
```
...
```bash
ollama run gemma3:12b "List exactly 3 safety hazards of MIG welding. Use numbered list format."
```
...
```bash
ollama run gemma3:12b "Describe the process."
```

Vague input = vague output. Compare that to:

```bash
ollama run gemma3:12b "Describe the process of performing a dye penetrant test on a stainless steel weld joint."
```
