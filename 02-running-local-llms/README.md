...
What do you see? You should get a table showing models you've already pulled. Look for `llama3.1:8b` -- that's your workhorse for this journey.
...
This downloads about 4.7GB. On your M4 Pro with 48GB RAM, this model will barely make a dent -- it uses about 5GB when running.
...
Look at the response. Notice how it gives you a pretty reasonable answer? That's an 8-billion-parameter model running *entirely on your laptop*. No internet needed, no API key, no bill at the end of the month.
...
Compare this output to what llama3.1:8b gave you. Notice the difference? The smaller model (3.8B parameters) is faster but the output is usually less detailed, less professional, and more likely to miss important things like safety requirements or documentation steps.
...
Run it. The response comes from your local llama3.1:8b, but the code looks *exactly* like it would for calling OpenAI's GPT-4.
...
| `llama3.1:8b` | 8B | ~5GB | Fast | Good | **Daily learning and development** |
| `qwen3:32b` | 32B | ~20GB | Slower | Excellent | When you need high-quality output |
| `llama4:scout` | 109B (MoE) | ~30GB | Slow | Excellent | Best local quality (mixture of experts) |

**Recommendation:** Use `llama3.1:8b` for all exercises in this learning journey. It's fast enough for rapid iteration and good enough to learn from. When you build your capstone project and need better quality, step up to `qwen3:32b` or `llama4:scout`.

Want to try the bigger model?

```bash
ollama pull qwen3:32b
```

Fair warning: it takes a while to download and runs noticeably slower. But the output quality jump is real. Try it on the same manufacturing prompt later and compare.
...
print("For learning and iteration: fast is better (llama3.1:8b).")
print("For final output quality: bigger is usually better (qwen3:32b).")
```
...
5. **Model size is a tradeoff** -- 8B for speed during development, 32B+ for quality in production
